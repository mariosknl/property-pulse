...
We are going to refactor the property edit page to a server component. Open the file `app/properties/edit/page.jsx` and replace the code with the following:
...
We are now querying the property from the database and passing it to the `PropertyEditForm` component.

Now open the `app/components/PropertyEditForm.jsx` file and replace it with the following:
...
We stripped all of the client-side stuff like `useState`, `useEffect`, etc. Right now, there is no form submission logic, but we will add that using an action soon.

You can now completely delete the `utils/requests.js` file as we are not going to use it anymore.
